...
import { useBlockUsersByEmail, useGetUserDetailsByEmail } from "@/utils/api/adminController/users";
...
const MerchantProfile = () => {
      ...
      const {mutate:blockUser} = useBlockUsersByEmail(email??"");
      ...
      const confirmFilter=()=>{
        ...
        navigate("/admindashboard/merchants");
      }
  ...
  return (
    <div className="w-full">
        <AdminPageHeader heading="" link="/admindashboard/merchants"/>
        <Title title="Merchant Profile" />
        ...
        {isSuccess && <div className="w-full">
          <Card className="md:mx-10 mt-6 min-h-36 text-[18px] sm:flex shadow-lg rounded-2xl gap-10 p-5 justify-between">
            ...
            <div>
                <Button
                  className="hover:cursor-pointer hover:bg-red-500 hover:text-white border-2 border-red-500 mt-4 p-1  px-4  md:text-[16px] font-primary-bold   md:block bg-white text-red-500" onClick={handleOpenCard}
                >
                  Block User
                </Button>                
            </div>
          ...
          <div className="flex  mx-4 md:mx-0 justify-between w-[80%] md:w-[30%] mt-4 md:ml-8 border-b-2 border-gray-400">
                <p className="text-[20px] font-medium">Total Amount Spent:</p>
      ...
      <CommonModalCard ref={modalref} title={modalContent.blockuser.title} description={modalContent.blockuser.description}>
                <Button
