...
import { useGetApplieOrder} from "@/utils/api/MerchantController/merchant";
import { getFormattedDate } from "@/utils/utils";
import { useNavigate } from "react-router-dom";
import image404 from "@/assets/images/404page.gif"
export const AppliedOrders = () => {
  const {data:orderList} = useGetApplieOrder();
    const navigate = useNavigate();
  return (
    <div className="w-[100%] h-[100%]">
      <div className="flex justify-between w-[91%] items-center mt-6">
        <div className="flex items-center">
          <Button className=" mx-10 bg-white border-2 border-secondary-green text-secondary-green hover:bg-gray-100 cursor-pointer" onClick={() => {navigate(-1)}}>
            {" "}
            {"< Back"}
          </Button>
          <h1 className="text-[32px] font-primary-bold text-text-grey">
            Applied Orders Details
          </h1>
        </div>
      </div>
          ...
          <Card key= {index} className="w-[95%] ml-6 mt-4  p-6 shadow items-center rounded-2xl mb-4 md:mb-0">
            ...
            <div className="flex justify-between items-center mt-4 w-[100%] ">
              ...
              <Button className="bg-secondary-green border-2 border-secondary-green text-white hover:bg-primary-green cursor-pointer text-[20px] h-[40px]" onClick={()=>{
                navigate(`/merchant/applied-order/${order.orderId}/${order.totalPrice}`)
              }}>
                View Details
              </Button>
            </div>
...
)}
{
  ...
  <h1 className=" font-bold text-2xl md:text-4xl">OOPS! No Applied  orders yet</h1>
